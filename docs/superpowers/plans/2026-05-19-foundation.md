...
## Task 7: Font assets and loading helper

**Files:**
- Create: `assets/fonts/` (font files placed by hand)
- Create: `src/theme/fonts.ts`

- [ ] **Step 1: Download the font files**

Download these static `.ttf` files and place them in `Stillpoint/assets/fonts/`:

From Google Fonts (`https://fonts.google.com/specimen/Fraunces` → "Get font" → download, extract the `static/` folder):
- `Fraunces-Light.ttf` (weight 300)
- `Fraunces-Regular.ttf` (weight 400)

From Google Fonts (`https://fonts.google.com/specimen/Raleway` → "Get font" → download, extract `static/`):
- `Raleway-Regular.ttf`
- `Raleway-Medium.ttf`
- `Raleway-SemiBold.ttf`

Verify: `ls assets/fonts/`
Expected: the five `.ttf` files listed.
...
```ts
/**
 * Maps the font-family names used in `typography.ts` to their .ttf assets.
 * Passed to `useFonts` in App.tsx. The keys MUST match `fontFamily` values.
 */
...
- [ ] **Step 3: Verify TypeScript accepts the requires**

Run: `npx tsc --noEmit`
Expected: exits 0. (Expo's `expo-env.d.ts` declares `.ttf` modules; if tsc errors on the require, ensure `expo-env.d.ts` exists — it is generated by the scaffold.)

- [ ] **Step 4: Commit**

```bash
git add assets/fonts src/theme/fonts.ts
git commit -m "feat: add Fraunces and Raleway font assets and loader map"
```
